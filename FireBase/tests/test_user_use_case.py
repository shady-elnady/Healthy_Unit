...
from FireBase.Use_Case.Abstract_FireBase_User_UseCase import AbstractFireBaseUserUseCase
from ..Use_Case.FireBase_User_UseCase import FireBaseUserUseCase
...
def test_create_customer_use_case(
    customer_factory, customer_repo: AbstractFireBaseUserUseCase
):
    customer_use_case = FireBaseUserUseCase(customer_repo=customer_repo)
    customer_data = customer_factory.build()
    customer_entity = FireBaseUserEntity(**customer_data.__dict__)
...
def test_get_customer_use_case(
    customer_factory, customer_repo: AbstractFireBaseUserUseCase
):
    customer_use_case = FireBaseUserUseCase(customer_repo=customer_repo)
    customer_data = customer_factory()
...
def test_list_customer_use_case(
    customer_factory, customer_repo: AbstractFireBaseUserUseCase
):
    customer_use_case = FireBaseUserUseCase(customer_repo=customer_repo)
